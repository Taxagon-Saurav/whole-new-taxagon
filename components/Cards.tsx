...
export function BenefitCard({ icon, title, description }: CardProps) {
  return (
    <Reveal className="card-hover rounded-[2rem] border border-slate-200 bg-white/82 p-6 shadow-sm backdrop-blur">
      <div className="mb-6 grid h-14 w-14 place-items-center rounded-2xl bg-blue-50 text-2xl font-bold text-[#1C41F7]">{icon}</div>
  ...
  return (
    <Reveal className="group card-hover flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-7 flex items-center justify-between">
        <div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#1C41F7] text-xl font-bold text-white shadow-lg shadow-[#1C41F7]/25">{icon}</div>
        <span className="text-2xl text-slate-300 transition group-hover:translate-x-1 group-hover:text-[#1C41F7]">→</span>
      </div>
      <h3 className="text-2xl font-bold tracking-[-0.04em] text-slate-950">{title}</h3>
...
export function TestimonialCard({ quote, name, role }: { quote: string; name: string; role: string }) {
  return (
    <article className="min-w-[310px] rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:min-w-[380px]">
      <div className="text-5xl font-bold leading-none text-[#1C41F7]">“</div>
      <p className="mt-2 text-lg font-bold leading-8 tracking-[-0.02em] text-slate-950">{quote}</p>
      <div className="mt-7 flex items-center gap-3">
        <div className="grid h-11 w-11 place-items-center rounded-full bg-slate-950 text-sm font-bold text-white">{name.slice(0, 1)}</div>
        <div>
          <p className="m-0 font-bold text-slate-950">{name}</p>
...
export function TextLink({ href, children }: { href: string; children: ReactNode }) {
  return <Link href={href} className="font-bold text-[#1C41F7] underline decoration-[#1C41F7]/20 underline-offset-4 hover:decoration-[#1C41F7]">{children}</Link>;
}
