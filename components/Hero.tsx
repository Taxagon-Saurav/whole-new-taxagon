  ...
  return (
    <section className={`relative overflow-hidden ${compact ? 'py-20' : 'py-20 lg:py-28'}`}>
      ...
      <div className="container-shell grid gap-12 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
        <Reveal>
          <p className="mb-4 text-xs font-bold uppercase tracking-[0.24em] text-[#1C41F7]">{eyebrow}</p>
          <h1 className="text-balance text-5xl font-bold leading-[0.95] tracking-[-0.075em] text-slate-800 sm:text-6xl lg:text-7xl xl:text-8xl">{title}</h1>
        ...
        <Reveal delay={0.12} className="relative">
          <div className="float-soft rounded-[2.5rem] border border-white bg-white/86 p-5 shadow-2xl shadow-slate-950/10 backdrop-blur">
            <div className="rounded-[2rem] bg-slate-950 p-5 text-white">
              <div className="flex items-center justify-between"><span className="text-sm font-bold text-slate-300">Taxagon snapshot</span><span className="rounded-full bg-emerald-400/15 px-3 py-1 text-xs font-bold text-emerald-200">Healthy</span></div>
              <div className="mt-8 rounded-3xl bg-white/10 p-5"><p className="text-sm text-slate-300">Estimated tax opportunities</p><strong className="mt-2 block text-4xl font-bold tracking-[-0.05em]">Planned early</strong></div>
